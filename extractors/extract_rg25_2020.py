"""Pipeline de producción para ingestar artículos de la Resolución General IGJ 25/2020
en Neo4j. La RG regula la adecuación estatutaria de Clubes de Campo y conjuntos
inmobiliarios preexistentes a la entrada en vigencia del Código Civil y Comercial de la
Nación (art. 2075), bajo la fiscalización de la IGJ. La persona jurídica que nuclea a los
propietarios (normalmente AsociacionCivil) debe dictar un Reglamento de Adecuación.
...
_CANONICOS: dict[str, str] = {
    ...
    "REGISTRO_PUBLICOS":          "REGISTRO_PUBLICO",
}
...
_log_file = os.path.join(BASE_DIR, "logs", f"rg25_2020_extract_{time.strftime('%Y%m%d_%H%M%S')}.log")
logging.basicConfig(
    level=logging.INFO,
...
logger = logging.getLogger("RG_25_2020")
...
def crear_nodo_madre(db_driver):
    with db_driver.session() as session:
        session.run("""
            MERGE (n:Norma {id: 'RG_25_2020'})
            SET n.numero        = '25/2020',
                n.titulo        = 'Resolución General IGJ 25/2020 - Adecuación estatutaria de Clubes de Campo y conjuntos inmobiliarios preexistentes',
                n.fecha_sancion = '18/05/2020',
        ...
        """)
        session.run("""
            MATCH (rg:Norma  {id: 'RG_25_2020'})
            MATCH (ley:Norma {id: 'Ley_22315'})
            MERGE (rg)-[:REGLAMENTA_TRAMITES]->(ley)
        """)
    logger.info("Nodo madre RG_25_2020 y vínculo normativo creados/verificados en Neo4j.")
...
def guardar_en_neo4j(grafo_extraido: GrafoLegalExtraido, db_driver, embedder):
    with db_driver.session() as session:
        ...
        for nodo in grafo_extraido.nodos:
            if nodo.etiqueta == "Articulo":
                session.run("""
                    MATCH (norma:Norma {id: 'RG_25_2020'})
                    MATCH (art {id: $art_id})
...
carpeta_archivos    = os.path.join(BASE_DIR, "input/normas/sociedades/RG/2020/articulos_RG25-2020/*.txt")
...
for file_path in archivos_a_procesar:
    ...
    articulo_id = f"Art_{numero}_RG_25_2020"
...
4. NODO ARTÍCULO:
   - ID: "Art_[Numero]_RG_25_2020"  (ej: "Art_1_RG_25_2020")
   - NO rellenes la propiedad 'texto' — quedará null.
   - 'ubicacion': dejar vacío (la norma no tiene estructura LIBRO/TÍTULO/CAPÍTULO).

5. Norma: Resolución General IGJ N° 25/2020. Regula la adecuación estatutaria de Clubes de
   Campo y conjuntos inmobiliarios preexistentes al Código Civil y Comercial de la Nación
   (art. 2075). La persona jurídica que nuclea a los propietarios (AsociacionCivil) debe
   dictar un Reglamento de Adecuación. La IGJ fiscaliza el proceso.
