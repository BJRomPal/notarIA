"""Pipeline de producción para ingestar artículos de la Resolución General IGJ 16/2023
en Neo4j. La RG establece que la SAU y la SAS no encuadran como asociaciones profesionales
(art. 1°) y habilita a la IGJ a transformar o subsanar —según pautas del CPCE CABA— a las
sociedades profesionales que lo soliciten, incluyendo las comprendidas en la Sección IV del
Cap. I de la Ley 19.550 y las sociedades civiles del derogado Código Civil (art. 2°).

Jerarquía normativa:
  Ley_22315  (Ley Orgánica IGJ)
  Ley_19550  (LSC — Sección IV Cap. I)
  Ley_27349  (SAS)
       ↑ REGLAMENTA_TRAMITES
  RG_16_2023
"""
...
_CANONICOS: dict[str, str] = {
    "SOCIEDAD_ANONIMA_UNIPERSONALES":        "SOCIEDAD_ANONIMA_UNIPERSONAL",
...
_log_file = os.path.join(BASE_DIR, "logs", f"rg16_2023_extract_{time.strftime('%Y%m%d_%H%M%S')}.log")
logging.basicConfig(
    level=logging.INFO,
...
logger = logging.getLogger("RG_16_2023")
...
def crear_nodo_madre(db_driver):
    with db_driver.session() as session:
        session.run("""
            MERGE (n:Norma {id: 'RG_16_2023'})
            SET n.numero        = '16/2023',
                n.titulo        = 'Resolución General IGJ 16/2023 - SAU y SAS inhabilitadas como asociaciones profesionales; transformación y subsanación de sociedades profesionales',
                n.fecha_sancion = '05/12/2023',
        ...
        for ley_id in ["Ley_22315", "Ley_19550", "Ley_27349"]:
            session.run("""
                MATCH (rg:Norma  {id: 'RG_16_2023'})
                MATCH (ley:Norma {id: $ley_id})
                MERGE (rg)-[:REGLAMENTA_TRAMITES]->(ley)
            """, ley_id=ley_id)
    logger.info("Nodo madre RG_16_2023 y vínculos normativos creados/verificados en Neo4j.")
...
def guardar_en_neo4j(grafo_extraido: GrafoLegalExtraido, db_driver, embedder):
    with db_driver.session() as session:
        ...
        for nodo in grafo_extraido.nodos:
            if nodo.etiqueta == "Articulo":
                session.run("""
                    MATCH (norma:Norma {id: 'RG_16_2023'})
                    MATCH (art {id: $art_id})
...
carpeta_archivos    = os.path.join(BASE_DIR, "input/normas/sociedades/RG/2023/articulos_RG16-2023/*.txt")
...
for file_path in archivos_a_procesar:
    ...
    articulo_id = f"Art_{numero}_RG_16_2023"
...
1. ENTIDADES — USA ÚNICAMENTE estas etiquetas exactas (PascalCase):
   ...
   Ejemplo: etiqueta="SociedadAnonimaUnipersonal"    → id="SOCIEDAD_ANONIMA_UNIPERSONAL"
   Ejemplo: etiqueta="SociedadPorAccionesSimplificada" → id="SOCIEDAD_POR_ACCIONES_SIMPLIFICADA"
   IMPORTANTE: "asociaciones profesionales", "sociedades civiles", "CPCE" NO son entidades; no las extraigas.
...
4. NODO ARTÍCULO:
   - ID: "Art_[Numero]_RG_16_2023"  (ej: "Art_1_RG_16_2023")
   - NO rellenes la propiedad 'texto' — quedará null.
   - 'ubicacion': dejar vacío (la norma no tiene estructura LIBRO/TÍTULO/CAPÍTULO).

5. Norma: Resolución General IGJ N° 16/2023. Art. 1°: la SAU y la SAS no encuadran
   como asociaciones profesionales. Art. 2°: la IGJ transformará o subsanará —según
   pautas del CPCE CABA— a las sociedades profesionales que lo soliciten, incluyendo
   las irregulares de la Sección IV Cap. I Ley 19.550 y las civiles del Código Civil.
