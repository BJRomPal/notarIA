"""Pipeline de producción para ingestar artículos de la Resolución General IGJ 30/2020
en Neo4j. La RG establece un procedimiento especial de inscripción urgente (5 días hábiles)
para constituciones de SRL, SA y SAU, con opción de solicitar simultáneamente la rúbrica
de libros contables y societarios. Incluye la entrega del CUIT con la certificación de
inscripción mediante el formulario AFIP 185.
Deroga las RG IGJ N° 5/2017 y 1/2018.

Jerarquía normativa:
  Ley_22315  (Ley Orgánica IGJ)
  Ley_19550  (SA / SRL / SAU)
       ↑ REGLAMENTA_TRAMITES
  RG_30_2020
"""
...
_log_file = os.path.join(BASE_DIR, "logs", f"rg30_2020_extract_{time.strftime('%Y%m%d_%H%M%S')}.log")
logging.basicConfig(
    level=logging.INFO,
...
logger = logging.getLogger("RG_30_2020")
...
def crear_nodo_madre(db_driver):
    with db_driver.session() as session:
        session.run("""
            MERGE (n:Norma {id: 'RG_30_2020'})
            SET n.numero        = '30/2020',
                n.titulo        = 'Resolución General IGJ 30/2020 - Inscripción urgente y rúbrica simultánea de libros para SRL, SA y SAU',
                n.fecha_sancion = '31/07/2020',
        ...
        for ley_id in ["Ley_22315", "Ley_19550"]:
            session.run("""
                MATCH (rg:Norma  {id: 'RG_30_2020'})
                MATCH (ley:Norma {id: $ley_id})
                MERGE (rg)-[:REGLAMENTA_TRAMITES]->(ley)
            """, ley_id=ley_id)
    logger.info("Nodo madre RG_30_2020 y vínculos normativos creados/verificados en Neo4j.")
...
def guardar_en_neo4j(grafo_extraido: GrafoLegalExtraido, db_driver, embedder):
    with db_driver.session() as session:
        ...
        for nodo in grafo_extraido.nodos:
            if nodo.etiqueta == "Articulo":
                session.run("""
                    MATCH (norma:Norma {id: 'RG_30_2020'})
                    MATCH (art {id: $art_id})
...
carpeta_archivos    = os.path.join(BASE_DIR, "input/normas/sociedades/RG/2020/articulos_RG30-2020/*.txt")
...
for file_path in archivos_a_procesar:
    ...
    articulo_id = f"Art_{numero}_RG_30_2020"
...
4. NODO ARTÍCULO:
   - ID: "Art_[Numero]_RG_30_2020"  (ej: "Art_1_RG_30_2020")
   - NO rellenes la propiedad 'texto' — quedará null.
   - 'ubicacion': dejar vacío (la norma no tiene estructura LIBRO/TÍTULO/CAPÍTULO).

5. Norma: Resolución General IGJ N° 30/2020. Establece un procedimiento especial de
   inscripción urgente (5 días hábiles) para constituciones de SRL, SA y SAU, con opción
   de solicitar simultáneamente la rúbrica de libros contables y societarios. Incluye
   la entrega del CUIT con la certificación de inscripción (Formulario AFIP 185).
