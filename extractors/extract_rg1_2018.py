"""Pipeline de producción para ingestar artículos de la Resolución General IGJ 1/2018
en Neo4j. La RG establece un procedimiento especial de inscripción urgente (24 hs)
para constituciones de Sociedades Anónimas y Sociedades Anónimas Unipersonales,
complementando el régimen general de la RG IGJ 07/15 (hoy absorbida por RG_15_2024).

Jerarquía normativa:
  Ley_19550  (SA/SAU)
       ↑ REGLAMENTA_TRAMITES
  RG_1_2018  — trámite urgente de inscripción de SA/SAU ante IGJ
"""
...
_log_file = os.path.join(BASE_DIR, "logs", f"rg1_2018_extract_{time.strftime('%Y%m%d_%H%M%S')}.log")
logging.basicConfig(
    level=logging.INFO,
...
logger = logging.getLogger("RG_1_2018")
...
def crear_nodo_madre(db_driver):
    with db_driver.session() as session:
        session.run("""
            MERGE (n:Norma {id: 'RG_1_2018'})
            SET n.numero        = '1/2018',
                n.titulo        = 'Resolución General IGJ 1/2018 - Trámite urgente de constitución de SA y SAU',
                n.fecha_sancion = '14/02/2018',
        ...
        session.run("""
            ...
            MERGE (rg)-[:REGLAMENTA_TRAMITES]->(ley)
        """)
    logger.info("Nodo madre RG_1_2018 y vínculos normativos creados/verificados en Neo4j.")
...
def guardar_en_neo4j(grafo_extraido: GrafoLegalExtraido, db_driver, embedder):
    with db_driver.session() as session:
        ...
        for nodo in grafo_extraido.nodos:
            if nodo.etiqueta == "Articulo":
                session.run("""
                    MATCH (norma:Norma {id: 'RG_1_2018'})
                    MATCH (art {id: $art_id})
...
carpeta_archivos    = os.path.join(BASE_DIR, "input/normas/sociedades/RG/2018/articulos_RG1-2018/*.txt")
...
for file_path in archivos_a_procesar:
    ...
    articulo_id = f"Art_{numero}_RG_1_2018"
...
4. NODO ARTÍCULO:
   - ID: "Art_[Numero]_RG_1_2018"  (ej: "Art_1_RG_1_2018")
   - NO rellenes la propiedad 'texto' — quedará null.
   - 'ubicacion': dejar vacío (la norma no tiene estructura LIBRO/TÍTULO/CAPÍTULO).

5. Norma: Resolución General IGJ N° 1/2018 — dictada por el Inspector General de
   Justicia bajo autorización de la Ley N° 22.315. Regula el trámite urgente de
   inscripción de constituciones de SA y SAU ante la IGJ.
