"""Pipeline de producción para ingestar artículos de la Resolución General IGJ 45/2020
en Neo4j. La RG establece que los directores y gerentes suplentes pueden asumir
automáticamente en el órgano de administración colegiado —sin necesidad de una reunión
previa de toma de posesión— cuando el órgano no alcance el quórum necesario, siempre que
hayan cumplido con la garantía del art. 256, segundo párrafo, de la Ley 19.550.

Jerarquía normativa:
  Ley_22315  (Ley Orgánica IGJ)
  Ley_19550  (SA / SRL — arts. 256 y 260)
       ↑ REGLAMENTA_TRAMITES
  RG_45_2020
"""
...
_CANONICOS: dict[str, str] = {
    "DIRECTORES":                             "DIRECTOR",
    "DIRECTOR_SUPLENTE":                      "DIRECTOR",
    "DIRECTORES_SUPLENTES":                   "DIRECTOR",
    "GERENTE":                                "DIRECTOR",
    "GERENTES":                               "DIRECTOR",
    "GERENTE_SUPLENTE":                       "DIRECTOR",
    "GERENTES_SUPLENTES":                     "DIRECTOR",
...
_log_file = os.path.join(BASE_DIR, "logs", f"rg45_2020_extract_{time.strftime('%Y%m%d_%H%M%S')}.log")
logging.basicConfig(
    level=logging.INFO,
...
logger = logging.getLogger("RG_45_2020")
...
def crear_nodo_madre(db_driver):
    with db_driver.session() as session:
        session.run("""
            MERGE (n:Norma {id: 'RG_45_2020'})
            SET n.numero        = '45/2020',
                n.titulo        = 'Resolución General IGJ 45/2020 - Asunción automática de directores y gerentes suplentes en órganos de administración colegiados',
                n.fecha_sancion = '24/11/2020',
        ...
        for ley_id in ["Ley_22315", "Ley_19550"]:
            session.run("""
                MATCH (rg:Norma  {id: 'RG_45_2020'})
                MATCH (ley:Norma {id: $ley_id})
                MERGE (rg)-[:REGLAMENTA_TRAMITES]->(ley)
            """, ley_id=ley_id)
    logger.info("Nodo madre RG_45_2020 y vínculos normativos creados/verificados en Neo4j.")
...
def guardar_en_neo4j(grafo_extraido: GrafoLegalExtraido, db_driver, embedder):
    with db_driver.session() as session:
        ...
        for nodo in grafo_extraido.nodos:
            if nodo.etiqueta == "Articulo":
                session.run("""
                    MATCH (norma:Norma {id: 'RG_45_2020'})
                    MATCH (art {id: $art_id})
...
carpeta_archivos    = os.path.join(BASE_DIR, "input/normas/sociedades/RG/2020/articulos_RG45-2020/*.txt")
...
for file_path in archivos_a_procesar:
    ...
    articulo_id = f"Art_{numero}_RG_45_2020"
...
1. ENTIDADES — USA ÚNICAMENTE estas etiquetas exactas (PascalCase):
   ...
   Ejemplo: etiqueta="SociedadDeResponsabilidadLimitada" → id="SOCIEDAD_DE_RESPONSABILIDAD_LIMITADA"
   IMPORTANTE: los gerentes suplentes deben mapearse como Director (id="DIRECTOR").
...
4. NODO ARTÍCULO:
   - ID: "Art_[Numero]_RG_45_2020"  (ej: "Art_1_RG_45_2020")
   - NO rellenes la propiedad 'texto' — quedará null.
   - 'ubicacion': dejar vacío (la norma no tiene estructura LIBRO/TÍTULO/CAPÍTULO).

5. Norma: Resolución General IGJ N° 45/2020. Establece que los directores y gerentes
   suplentes pueden asumir automáticamente en el órgano de administración colegiado
   sin necesidad de una reunión previa de toma de posesión, cuando no se alcance el
   quórum necesario. Aplica a SA y SRL. Referencias a arts. 256 y 260 de Ley 19.550.
