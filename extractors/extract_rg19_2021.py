"""Pipeline de producción para ingestar artículos de la Resolución General IGJ 19/2021
en Neo4j. La RG regula la inscripción voluntaria del protocolo de empresa familiar en
el Registro Público a cargo de la IGJ para sociedades comerciales. Define qué se entiende
por protocolo de empresa familiar, las condiciones de inscripción, sus efectos (dar
publicidad a terceros, sin presunción de legalidad) y el trámite requerido (dictamen de
precalificación profesional, conformidad de todos los socios).
...
_log_file = os.path.join(BASE_DIR, "logs", f"rg19_2021_extract_{time.strftime('%Y%m%d_%H%M%S')}.log")
logging.basicConfig(
    level=logging.INFO,
...
logger = logging.getLogger("RG_19_2021")
...
def crear_nodo_madre(db_driver):
    with db_driver.session() as session:
        session.run("""
            MERGE (n:Norma {id: 'RG_19_2021'})
            SET n.numero        = '19/2021',
                n.titulo        = 'Resolución General IGJ 19/2021 - Inscripción del protocolo de empresa familiar en el Registro Público',
                n.fecha_sancion = '17/12/2021',
        ...
        """)
        session.run("""
            MATCH (rg:Norma  {id: 'RG_19_2021'})
            MATCH (ley:Norma {id: 'Ley_22315'})
            MERGE (rg)-[:REGLAMENTA_TRAMITES]->(ley)
        """)
    logger.info("Nodo madre RG_19_2021 y vínculo normativo creados/verificados en Neo4j.")
...
def guardar_en_neo4j(grafo_extraido: GrafoLegalExtraido, db_driver, embedder):
    with db_driver.session() as session:
        ...
        for nodo in grafo_extraido.nodos:
            if nodo.etiqueta == "Articulo":
                session.run("""
                    MATCH (norma:Norma {id: 'RG_19_2021'})
                    MATCH (art {id: $art_id})
...
carpeta_archivos    = os.path.join(BASE_DIR, "input/normas/sociedades/RG/2021/articulos_RG19-2021/*.txt")
...
for file_path in archivos_a_procesar:
    ...
    articulo_id = f"Art_{numero}_RG_19_2021"
...
1. ENTIDADES — USA ÚNICAMENTE estas etiquetas exactas (PascalCase):
   ...
   Ejemplo: etiqueta="RegistroPublico"              → id="REGISTRO_PUBLICO"
   Ejemplo: etiqueta="InspeccionGeneralDeJusticia"  → id="INSPECCION_GENERAL_DE_JUSTICIA"
   IMPORTANTE: "protocolo de empresa familiar", "socios", "órgano de administración"
   NO son entidades de la lista; no los extraigas como nodos.
...
4. NODO ARTÍCULO:
   - ID: "Art_[Numero]_RG_19_2021"  (ej: "Art_1_RG_19_2021")
   - NO rellenes la propiedad 'texto' — quedará null.
   - 'ubicacion': dejar vacío (la norma no tiene estructura LIBRO/TÍTULO/CAPÍTULO).

5. Norma: Resolución General IGJ N° 19/2021. Regula la inscripción voluntaria del
   protocolo de empresa familiar en el Registro Público de la IGJ para sociedades
   comerciales. Define el concepto de protocolo, exige escritura pública o instrumento
   privado con firmas certificadas, requiere dictamen de precalificación profesional y
   fija como único efecto dar publicidad a terceros sin presunción de legalidad.
